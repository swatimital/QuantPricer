...
namespace QuantPricer
{
    namespace UncertainVolatility
    {
        ...
        boost::shared_ptr<FiniteDiffMethods::Node<double, Equities::BarenblattDerivative>> BarenblattTrinomialTree::BuildUnderlyingTree(double val, NodeDir ndir, int tree_level)
        {
            ...
            if (ndir == NodeDir::Up) {
                nd = boost::make_shared<FiniteDiffMethods::Node<double, Equities::BarenblattDerivative>>(m_up_factor*val, Equities::BarenblattDerivative(std::make_tuple<double, double>(-1.0,-1.0)));
            } else if(ndir == NodeDir::Down) {
                nd = boost::make_shared<FiniteDiffMethods::Node<double, Equities::BarenblattDerivative>>(m_down_factor*val, Equities::BarenblattDerivative(std::make_tuple<double, double>(-1.0,-1.0)));
            } else {
                nd = boost::make_shared<FiniteDiffMethods::Node<double, Equities::BarenblattDerivative>>(m_middle_factor*val, Equities::BarenblattDerivative(std::make_tuple<double, double>(-1.0,-1.0)));
            }
